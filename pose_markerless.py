...
def head_point(kp, threshold=0.5):
    """Best available head location: nose if visible, else the midpoint of the
    visible ears, else None. Shared by the analysis and the 14-keypoint bootstrap
    so 'head_center' is defined the same way everywhere."""
    if kp[0, 2] >= threshold:
        return kp[0, :2].copy()
    ears = [kp[i, :2] for i in (3, 4) if kp[i, 2] >= threshold]
    if ears:
        return np.mean(ears, axis=0)
    return None
...
def detect_boxes(model, frame, imgsz, conf):
    r = model.predict(frame, imgsz=imgsz, conf=conf, verbose=False)[0]
    if r.boxes is None or len(r.boxes) == 0:
        return []
    return [tuple(map(float, b)) for b in r.boxes.xywh.cpu().numpy()]


def select_swimmer(boxes, W, H, prev=None, min_frac=0.02):
    """Pick the swimmer robustly. Ignore implausibly small detections (the mask,
    a hand, a reflection). Once we have locked on, prefer the detection nearest
    the previous swimmer position rather than the largest box, so identity does
    not jump to the equipment."""
    if not boxes:
        return None
    big = [b for b in boxes if b[2] * b[3] >= min_frac * W * H]
    cand = big if big else boxes
    if prev is None:
        return max(cand, key=lambda b: b[2] * b[3])
    return min(cand, key=lambda b: (b[0] - prev[0]) ** 2 + (b[1] - prev[1]) ** 2)


def pose_cropped(model, frame, imgsz=640, conf=0.25, margin=0.28,
                 use_clahe=True, rotate=True, prev_box=None):
    ...
    coordinates. Returns (keypoints, chosen_box); pass chosen_box back in as
    prev_box on the next frame to keep tracking the same swimmer."""
    H, W = frame.shape[:2]
    boxes = detect_boxes(model, frame, imgsz, conf)
    box = select_swimmer(boxes, W, H, prev_box)
    if box is None:
        return None, None
    x, y, w, h = box
...
def quality_filter(kp, conf_min=0.6):
    ...
    # 2) left/right joints collapsing onto the same point: the model has put both
    #    limbs on one (overlapping limbs). Keep the more confident one rather than
    #    deleting both, so at least the real limb still shows and gets measured.
    for l, r in [(9, 10), (15, 16), (13, 14), (7, 8)]:
        pl, pr = pt(l), pt(r)
        if pl is not None and pr is not None and np.linalg.norm(pl - pr) < 0.08 * torso:
            drop = l if kp[l, 2] < kp[r, 2] else r
            kp[drop, 2] = 0.0
